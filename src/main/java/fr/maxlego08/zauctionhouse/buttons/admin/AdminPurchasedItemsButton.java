...
public class AdminPurchasedItemsButton extends PaginateButton {
   ...
   public void onRender(Player var1, InventoryEngine var2) {
      ...
      if (var3.isEmpty()) {
         ...
         List var5 = var4.getPurchasedItems((UUID)var3.get());
         this.paginate(var5, var2, (var4x, var5x) -> var2.addItem(var4x, var5x.buildItemStack(var1)).setClick((var4xx) -> var4.adminRemoveItem(var1, (UUID)var3.get(), var5x, StorageType.PURCHASED)));
      }
   }

   public int getPaginationSize(@NonNull Player var1) {
      return (Integer)this.getTarget(var1).map((var1x) -> this.plugin.getAuctionManager().getPurchasedItems(var1x).size()).orElse(0);
   }
