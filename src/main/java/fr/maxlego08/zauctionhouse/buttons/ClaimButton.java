...
public class ClaimButton extends Button {
      ...
      if (!var4.has(PlayerCacheKey.PENDING_MONEY_DATA)) {
         ...
         if (!var5) {
            ...
            this.plugin.getAuctionManager().getClaimService().getPendingMoneyByEconomy(var1.getUniqueId()).thenAccept((var4x) -> {
               ...
               this.plugin.getScheduler().runAtEntity(var1, (var3x) -> {
                  if (var1.isOnline()) {
   ...
   public ItemStack getCustomItemStack(@NotNull Player var1, boolean var2, @NotNull Placeholders var3) {
      ...
      if ((Boolean)var4.get(PlayerCacheKey.PENDING_MONEY_LOADING, false)) {
         ...
         Map<String, BigDecimal> var6 = (Map<String, BigDecimal>)var4.get(PlayerCacheKey.PENDING_MONEY_DATA);
         BigDecimal var7 = BigDecimal.ZERO;
         if (var6 != null && !var6.isEmpty()) {
            var7 = var6.values().stream().reduce(BigDecimal.ZERO, BigDecimal::add);
         }

         for(AuctionEconomy var9 : var5.getEconomies()) {
            BigDecimal var10 = var6 != null ? var6.getOrDefault(var9.getName(), BigDecimal.ZERO) : BigDecimal.ZERO;
            var3.register("pending_" + var9.getName(), var5.format(var9, var10));
         }

         Collection<AuctionEconomy> var11 = var5.getEconomies();
         if (!var11.isEmpty()) {
            AuctionEconomy var12 = var11.iterator().next();
            var3.register("pending_total", var5.format(var12, var7));
         } else {
   ...
   public void onClick(@NonNull Player var1, @NonNull InventoryClickEvent var2, @NonNull InventoryEngine var3, int var4, @NonNull Placeholders var5) {
      ...
      Map<String, BigDecimal> var7 = (Map<String, BigDecimal>)var6.get(PlayerCacheKey.PENDING_MONEY_DATA);
      if (var7 != null && !var7.isEmpty()) {
         BigDecimal var8 = var7.values().stream().reduce(BigDecimal.ZERO, BigDecimal::add);
         if (var8.compareTo(BigDecimal.ZERO) > 0) {
            var6.remove(PlayerCacheKey.PENDING_MONEY_DATA, PlayerCacheKey.PENDING_MONEY_LOADING);
            this.plugin.getAuctionManager().getClaimService().claimMoney(var1).thenRun(() -> this.plugin.getScheduler().runAtEntity(var1, (var2x) -> {
                  if (var1.isOnline()) {
