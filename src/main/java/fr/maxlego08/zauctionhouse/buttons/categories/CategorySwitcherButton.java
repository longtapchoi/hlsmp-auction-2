...
public class CategorySwitcherButton extends Button {
   ...
   public ItemStack getCustomItemStack(@NotNull Player var1, boolean var2, @NotNull Placeholders var3) {
      ...
      Category var6 = var5.get(PlayerCacheKey.CURRENT_CATEGORY);
      ...
      this.categories.forEach((var4x) -> {
         String var5x = var8.getCategory(var4x).map(Category::getDisplayName).orElse(this.plugin.getCategoryManager().getAllCategoryName());
         String var6x = (var4x.equalsIgnoreCase(var7) ? this.enableText : this.disableText).replace("%category%", var5x);
         var3.register(var4x, var6x);
      });
   ...
   public void onClick(@NonNull Player var1, @NonNull InventoryClickEvent var2, @NonNull InventoryEngine var3, int var4, @NonNull Placeholders var5) {
      ...
      if (!this.categories.isEmpty()) {
         ...
         Category var7 = var6.get(PlayerCacheKey.CURRENT_CATEGORY);
         String var8 = var7 != null ? var7.getId() : "main";
         int var9 = -1;

         for(int var10 = 0; var10 < this.categories.size(); ++var10) {
            if (this.categories.get(var10).equalsIgnoreCase(var8)) {
               var9 = var10;
               break;
            }
         }
         ...
         String var12 = this.categories.get(var11);
         if (var12.equalsIgnoreCase("main")) {
            ...
            Optional<Category> var14 = var13.getCategory(var12);
            if (var14.isEmpty()) {
               this.plugin.getLogger().warning("Category not found: " + var12);
               return;
            }

            var6.set(PlayerCacheKey.CURRENT_CATEGORY, var14.get());
         }
