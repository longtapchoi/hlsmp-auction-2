...
public class ClaimService extends AuctionService implements AuctionClaimService {
   ...
   public CompletableFuture<ClaimResult> claimMoney(Player var1) {
      return this.getPendingTransactions(var1.getUniqueId()).thenApply((var2) -> {
         if (var2.isEmpty()) {
            ...
            Map var3 = (Map)var2.stream().collect(Collectors.groupingBy(TransactionDTO::economy_name));
            if (var3.isEmpty()) {
               this.message(this.plugin, var1, Message.CLAIM_NO_PENDING, new Object[0]);
               return ClaimResult.nothingToClaim("No pending transactions");
            } else {
               EconomyManager var4 = this.plugin.getEconomyManager();
               List var5 = var2.stream().map(TransactionDTO::id).toList();
               ...
               for(Map.Entry var10 : var3.entrySet()) {
                  String var11 = (String)var10.getKey();
                  List var12 = (List)var10.getValue();
                  Optional var13 = var4.getEconomy(var11);
                  if (var13.isEmpty()) {
                     this.plugin.getLogger().warning("Economy not found: " + var11);
                  } else {
                     AuctionEconomy var14 = (AuctionEconomy)var13.get();
                     var8 = var14;
                     BigDecimal var15 = (BigDecimal)var12.stream().map(TransactionDTO::value).filter((var0) -> var0.compareTo(BigDecimal.ZERO) > 0).reduce(BigDecimal.ZERO, BigDecimal::add);
                     if (var15.compareTo(BigDecimal.ZERO) > 0) {
                        if (var1.isOnline()) {
                           this.plugin.getScheduler().runAsync((var4x) -> var14.deposit(var1.getUniqueId(), var15, var6));
                           this.message(this.plugin, var1, Message.CLAIM_ECONOMY_SUCCESS, new Object[]{"%amount%", var4.format((AuctionEconomy)var14, var15), "%economy%", var14.getDisplayName()});
                        }
      ...
      return this.getPendingTransactions(var1).thenApply((var0) -> {
         ...
         for(TransactionDTO var3 : var0) {
            if (var3.value().compareTo(BigDecimal.ZERO) > 0) {
               var1.merge(var3.economy_name(), var3.value(), BigDecimal::add);
            }
         }
   ...
   public CompletableFuture<BigDecimal> getTotalPendingMoney(UUID var1) {
      return this.getPendingMoneyByEconomy(var1).thenApply((var0) -> (BigDecimal)var0.values().stream().reduce(BigDecimal.ZERO, BigDecimal::add));
   }
      ...
      return this.getPendingTransactions(var1).thenAccept((var3) -> {
         if (!var3.isEmpty()) {
            if (var2) {
               ...
               Map var6 = (Map)var3.stream().collect(Collectors.groupingBy(TransactionDTO::economy_name));

               for(Map.Entry var8 : var6.entrySet()) {
                  Optional var9 = var4.getEconomy((String)var8.getKey());
                  if (var9.isEmpty()) {
                     this.plugin.getLogger().warning("Economy not found: " + (String)var8.getKey());
                  } else {
                     AuctionEconomy var10 = (AuctionEconomy)var9.get();
                     BigDecimal var11 = (BigDecimal)((List)var8.getValue()).stream().map(TransactionDTO::value).filter((var0) -> var0.compareTo(BigDecimal.ZERO) > 0).reduce(BigDecimal.ZERO, BigDecimal::add);
