...
public class ExpireService implements AuctionExpireService {
   ...
   public void processExpiredItem(Item var1, StorageType var2) {
      if (var1.getStatus() == ItemStatus.DELETED) {
         ...
         if (var2 == StorageType.LISTED) {
            ...
            Consumer<Long> var10 = (var3x) -> this.plugin.getScheduler().runNextTick((var4x) -> {
                  long var5L = var3x > 0L ? System.currentTimeMillis() + var3x * 1000L : 0L;
                  var1.setExpiredAt(new Date(var5L));
   ...
   public void processExpiredItems(List<Item> var1, StorageType var2) {
      if (!var1.isEmpty()) {
         ArrayList<Item> var3 = new ArrayList<>();
         ...
         if (!var3.isEmpty()) {
            ...
            this.auctionManager.clearPlayersCache(PlayerCacheKey.ITEMS_LISTED, PlayerCacheKey.ITEMS_SEARCH);
            HashSet<OfflinePlayer> var7 = new HashSet<>();

            for(Item var9 : (List<Item>)var3) {
            ...
            if (var2 == StorageType.LISTED) {
               ArrayList<Item> var19 = new ArrayList<>();
               ArrayList<Item> var22 = new ArrayList<>();
               ...
               if (!var19.isEmpty()) {
                  ...
                  EnumMap<StorageType, List<Item>> var26 = new EnumMap<>(StorageType.class);
               ...
               if (!var22.isEmpty()) {
                  ...
                  CopyOnWriteArrayList<Item> var30 = new CopyOnWriteArrayList<>();
                  ...
                  for(Item var32 : var22) {
                     var17.getExpireExpiration().getExpiration(this.plugin.getOfflinePermission(), var32.getSeller()).whenComplete((var6, var7x) -> {
                        ...
                        if (var27.decrementAndGet() == 0) {
                           this.plugin.getScheduler().runNextTick((var2z) -> {
                              EnumMap<StorageType, List<Item>> var3z = new EnumMap<>(StorageType.class);
                              var3z.put(StorageType.EXPIRED, new ArrayList<>(var30));
                              var18.updateItems(var3z);
                           });
                        }
               ...
               EnumMap<StorageType, List<Item>> var21 = new EnumMap<>(StorageType.class);
