   ...
   public CompletableFuture<PurchaseResult> purchaseItem(Player var1, Item var2) {
      ...
      if (!var3.callEvent()) {
         ...
         if (var9.giveItem() && var9.freeSpace() && !var2.canReceiveItem(var1)) {
            ...
            AtomicReference<fr.maxlego08.zauctionhouse.api.cluster.LockToken> var10 = new AtomicReference<>(null);
            AtomicReference<PurchaseResult> var11 = new AtomicReference<>(null);
            AtomicReference<ItemStatus> var12 = new AtomicReference<>(var2.getStatus());
            ...
            return var6.checkAvailability(var2).orTimeout(var13.checkAvailabilityTimeoutMs(), TimeUnit.MILLISECONDS).thenCompose((var7x) -> {
               ...
               if (LockToken.noop().value().equals(var10x.value())) {
                  ...
                  return var6.notifyItemStatusChange(var2, var12.get(), ItemStatus.IS_BEING_PURCHASED).orTimeout(var13.notifyStatusChangeTimeoutMs(), TimeUnit.MILLISECONDS).thenCompose((var3z) -> var8.has(var1.getUniqueId(), var2.getPrice()));
               }
            }).thenCompose((var8x) -> {
               LockToken var9x = var10.get();
               if (var8x) {
                  return var4.purchaseItem(var1, var2).thenCompose((var4x) -> var6.notifyItemBought(var1, var2).orTimeout(var13.notifyItemActionTimeoutMs(), TimeUnit.MILLISECONDS)).thenCompose((var4x) -> var6.unlockItem(var2, var9x, StorageType.LISTED).orTimeout(var13.unlockItemTimeoutMs(), TimeUnit.MILLISECONDS)).thenApply((var1x) -> {
                  ...
                  return var6.unlockItem(var2, var9x, StorageType.LISTED).orTimeout(var13.unlockItemTimeoutMs(), TimeUnit.MILLISECONDS).thenApply((var1x) -> (PurchaseResult)var11.get());
               }
            }).exceptionally((var6x) -> {
               ...
               if (var7x != null && !LockToken.noop().value().equals(var7x.value())) {
                  var6.unlockItem(var2, var7x, StorageType.LISTED).exceptionally((errUnlock) -> {
                     var7.severe("Failed to unlock item after error: " + errUnlock.getMessage());
                     return null;
                  });
               }

               if (var2.getStatus() == ItemStatus.IS_BEING_PURCHASED) {
                  ItemStatus var8x = var12.get();
                  var2.setStatus(var8x);
                  var6.notifyItemStatusChange(var2, ItemStatus.IS_BEING_PURCHASED, var8x).exceptionally((var2x) -> {
                     int var10001 = var2.getId();
               ...
               PurchaseResult var9r = var11.get();
               return var9r != null ? var9r : PurchaseResult.failure("Internal error", PurchaseFailReason.INTERNAL_ERROR);
            });
         }
      }
   }
