...
public record PermissionConfiguration(EnumMap<ItemType, Map<String, Integer>> permissions) {
   public static PermissionConfiguration of(AuctionPlugin var0, FileConfiguration var1) {
      ...
      for(ItemType var6 : ItemType.values()) {
         List var7 = var1.getMapList("permissions." + var6.name().toLowerCase());
         HashMap var8 = new HashMap();

         for(Map var10 : var7) {
            TypedMapAccessor var11 = new TypedMapAccessor(var10);
   ...
   public int getLimit(ItemType var1, Permissible var2) {
      return ((Map)this.permissions.get(var1)).entrySet().stream().filter((var1x) -> var2.hasPermission((String)var1x.getKey())).mapToInt(Map.Entry::getValue).max().orElse(0);
   }
}
