...
public record SearchFilterConfiguration(Map<SearchFilterType, String> operators) {
   ...
   public String getOperator(SearchFilterType var1) {
      return this.operators.getOrDefault(var1, var1.getDefaultOperator());
   }

   public Map.Entry<SearchFilterType, String>[] orderedByLength() {
      return this.operators.entrySet().stream().sorted((var0, var1) -> Integer.compare(var1.getValue().length(), var0.getValue().length())).toArray(Map.Entry[]::new);
   }
}
