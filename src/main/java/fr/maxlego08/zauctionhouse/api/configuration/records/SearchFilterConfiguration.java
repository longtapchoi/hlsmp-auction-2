...
public record SearchFilterConfiguration(Map<SearchFilterType, String> operators) {
   public static SearchFilterConfiguration of(AuctionPlugin var0, FileConfiguration var1) {
      TreeMap var2 = new TreeMap((var0x, var1x) -> Integer.compare(var1x.name().length(), var0x.name().length()));
   ...
   public Map.Entry<SearchFilterType, String>[] orderedByLength() {
      return (Map.Entry[])this.operators.entrySet().stream().sorted((var0, var1) -> Integer.compare(((String)var1.getValue()).length(), ((String)var0.getValue()).length())).toArray((var0) -> new Map.Entry[var0]);
   }
}
