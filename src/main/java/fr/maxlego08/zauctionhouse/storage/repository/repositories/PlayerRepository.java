...
public class PlayerRepository extends Repository {
   ...
   public String select(UUID var1) {
      return this.select(PlayerDTO.class, (var1x) -> var1x.where("unique_id", (Object)var1.toString())).stream().findFirst().map(PlayerDTO::name).orElse(null);
   }

   public UUID selectByName(String var1) {
      return this.select(PlayerDTO.class, (var1x) -> var1x.where("name", (Object)var1)).stream().findFirst().map(PlayerDTO::unique_id).orElse(null);
   }
}
