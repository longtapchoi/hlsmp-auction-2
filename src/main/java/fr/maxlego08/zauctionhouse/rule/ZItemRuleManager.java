...
public class ZItemRuleManager implements ItemRuleManager {
   ...
   private Rules loadRuleSet(FileConfiguration var1, String var2, boolean var3) {
      ...
      List<java.util.Map<?, ?>> var5 = var1.getMapList(var2 + ".rules");
      if (var5.isEmpty()) {
         return new Rules(var4, List.of());
      } else {
         ArrayList<fr.maxlego08.zauctionhouse.api.rules.Rule> var6 = new ArrayList<>();

         for(Map<?, ?> var8 : var5) {
            List<fr.maxlego08.zauctionhouse.api.rules.Rule> var9 = this.ruleLoaderRegistry.loadRules(var8);
            if (!var9.isEmpty()) {
               if (var9.size() == 1) {
